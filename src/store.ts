import { Hand, generateShoe, getHandTotal } from '@/blackjack';
...
const dealerTotal = computed(() => getHandTotal(dealer.value.hands[0].cards));
...
async function playHand(hand: Hand): Promise<void> {
  ...
  if (hand.cards.length === 1) { // Newly split hand
    await hit();
...
async function checkForBlackjack(hand: Hand): Promise<boolean> {
  if (hand.cards.length === 2 && getHandTotal(hand.cards) === 21) {
...
async function playDealerHand(hand: Hand) {
  ...
  const allPlayersDone = state.players.every(p => p.isDealer || p.hands.every((h: Hand) => !!h.result));
...
async function determineResults() {
  for (const player of state.players) {
    ...
    for (const hand of player.hands) {
      ...
      const handValue = getHandTotal(hand.cards);
      if (dealerTotal.value > 21) hand.result = 'win';
      else if (dealerTotal.value === handValue) hand.result = 'push';
      else if (dealerTotal.value < handValue) hand.result = 'win';
